...
binder<T,U,V,W,X>::binder(const std::string& name): _name(name), _ready(true) {
    ...
    _stars.emplace(0, new star(initial_switcher));
...
bool binder<T,U,V,W,X>::check_switcher(std::weak_ptr<switcher>& to_check, worm& to_update) {
    ...
    assert(_stars.find(_current_star.top()) != _stars.end() && _stars[_current_star.top()]); //current star is correct
...
void binder<T,U,V,W,X>::get_back(worm& to_update) {
    ...
    size_t top = _current_star.top();
    assert(_stars.find(top) != _stars.end() && _stars[top]);
    _stars[top] -> get_back();
...
typename binder<T,U,V,W,X>::level_id binder<T,U,V,W,X>::create_level(const U& value, const Y& data) {
    ...
    size_t i = _lowest_index(_free_levels);    
    _free_levels.emplace(i, std::make_shared<level> (value, initial_data));
...
void binder<T,U,V,W,X>::erase_level(const level_id& id) {
    ...
    _free_levels.erase(id._level_number);
...
void binder<T,U,V,W,X>::add_level(level_id& lid, channel_id& cid, size_t pos) {
    ...
    if (!initial -> get_newstar()) { // making new star 
        ...
        size_t position = _lowest_index(_stars);        
        _stars.emplace(position, new star(initial->get_edge().lock()));
        ...
        assert(_stars.find(newstar) != _stars.end() && _stars[newstar]);
...
void binder<T,U,V,W,X>::_lid_assert(const typename binder<T,U,V,W,X>::level_id& lid) {
    assert(_free_levels.find(lid._level_number) != _free_levels.end());
...
template <typename T, typename U, typename V, typename W, typename X>
template <typename Y>
size_t binder<T,U,V,W,X>::_lowest_index(const std::map<size_t, Y>& container) {
    size_t i = 1;
    while (i && container.find(i) != container.end())
        ++i;
    if (!i)
